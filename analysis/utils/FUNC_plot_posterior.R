...
plotPosteriorDimension = function(
  ...
  if(!is.null(dropParamsDf)){

    dropParamsDf = dplyr::mutate(dropParamsDf, logBeta=log(Rate_0_Sporulation))

    p = p +
        geom_point(data = dropParamsDf, aes_string(x=xKey, y=yKey), col = 'black', pch=4, size=2)
...
plotPosterior = function(
  ...
  dropParamsDf = NULL
  if(!is.null(dropSimsDf)){
      dropParamsDf = bigMinimalDfRaw[bigMinimalDfRaw$simKey%in%dropSimsDf$passKeys,]
  }
