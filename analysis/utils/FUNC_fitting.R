...
passGridFun = function(
    ...
    for(iRow in seq_len(nrow(configGridDf))){
        ...
        thisConfigRow = configGridDf[iRow,]

        thisTol = thisConfigRow$gridTol

        criteriaVec = thisConfigRow$criteria
        if(thisConfigRow$criteria=='all'){
        ...
        for(thisCriteria in criteriaVec){

            gridDfSubset = gridDf[gridDf$criteria==thisCriteria & gridDf$propFail<=thisTol,]
            
            gridKey = paste0("grid-", thisCriteria, "-", sprintf(thisTol, fmt = '%#.2f'))
            
            gridPassList[[gridKey]] = gridDfSubset$simKey
